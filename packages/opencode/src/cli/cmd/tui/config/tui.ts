...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  // Every config dir we may read from: global config dir, any `.bharatcode`
  // folders between cwd and home, and BHARATCODE_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.BHARATCODE_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit BHARATCODE_TUI_CONFIG override, if set.
  if (Flag.BHARATCODE_TUI_CONFIG) {
    const configFile = Flag.BHARATCODE_TUI_CONFIG
  ...
  // 4. `.bharatcode` directories (and BHARATCODE_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".bharatcode") || dir === Flag.BHARATCODE_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".bharatcode") && dir !== Flag.BHARATCODE_CONFIG_DIR) continue
