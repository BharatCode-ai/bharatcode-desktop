...
export const UninstallCommand = {
  command: "uninstall",
  describe: `uninstall ${DISTRIBUTION.displayName} and remove all related files`,
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro(`Uninstall ${DISTRIBUTION.displayName}`)
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    prompts.log.info(`  ✓ Package: ${Installation.uninstallCommand(method)?.join(" ") || method}`)
  }
}
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmd = Installation.uninstallCommand(method)
    if (cmd) {
