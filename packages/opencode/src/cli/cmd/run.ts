...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run BharatCode with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running BharatCode server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to BHARATCODE_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to BHARATCODE_SERVER_USERNAME or 'bharatcode')",
      })
      .option("dir", {
        type: "string",
