...
function normalizeLoadedConfig(data: unknown, source: string) {
  ...
  const copy = { ...data }
  if (copy.$schema === "https://opencode.ai/config.json") delete copy.$schema
  const hadLegacy = "theme" in copy || "keybinds" in copy || "tui" in copy
  if (!hadLegacy) return copy
  delete copy.theme
  delete copy.keybinds
  delete copy.tui
  log.warn("tui keys in BharatCode config are deprecated; move them to tui.json", { path: source })
  return copy
}
...
function globalConfigFile() {
  const candidates = ["bharatcode.jsonc", "bharatcode.json"].map((file) => path.join(Global.Path.config, file))
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      let result: Info = {}
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "bharatcode.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "bharatcode.jsonc"), env))
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "BHARATCODE_CONFIG_CONTENT") return "local"
        ...
        if (Flag.BHARATCODE_CONFIG) {
          yield* merge(Flag.BHARATCODE_CONFIG, yield* loadFile(Flag.BHARATCODE_CONFIG, authEnv))
          log.debug("loaded custom config", { path: Flag.BHARATCODE_CONFIG })
        }

        if (!Flag.BHARATCODE_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("bharatcode", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        if (Flag.BHARATCODE_CONFIG_DIR) {
          log.debug("loading config from BHARATCODE_CONFIG_DIR", { path: Flag.BHARATCODE_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".bharatcode") || dir === Flag.BHARATCODE_CONFIG_DIR) {
            for (const file of ["bharatcode.json", "bharatcode.jsonc"]) {
          ...
          // Auto-discovered plugins under `.bharatcode/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (Flag.BHARATCODE_CONFIG_CONTENT) {
          const source = "BHARATCODE_CONFIG_CONTENT"
          const next = yield* loadConfig(Flag.BHARATCODE_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          log.debug("loaded custom config from BHARATCODE_CONFIG_CONTENT")
        }
        ...
        if (process.env.OPENCODE_DISABLE_SHARE === "true" || process.env.OPENCODE_DISABLE_SHARE === "1") {
    ...
    const update = Effect.fn("Config.update")(function* (config: Info) {
      ...
      const file = path.join(dir, "bharatcode.json")
