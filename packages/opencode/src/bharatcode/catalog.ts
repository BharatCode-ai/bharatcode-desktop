...
// The catalog is served over an OpenAI-compatible surface, so both the provider
// and every model resolve through the openai-compatible AI SDK package.
const V2_API_PACKAGE = "@ai-sdk/openai-compatible"

export function toV2Provider(): ProviderV2.Info {
  return ProviderV2.Info.make({
    id: v2ProviderID,
    name: "BharatCode",
    api: {
      type: "aisdk",
      package: V2_API_PACKAGE,
...
export function toV2Model(model: Model): ModelV2.Info | undefined {
  const eligibility = codingEligibility(model)
  ...
  return ModelV2.Info.make({
    ...
    api: { id, type: "aisdk", package: V2_API_PACKAGE, settings: {} },
    capabilities: {
    ...
    request: { headers: {}, body: {} },
    variants: [],
    time: { released: (model.created ?? 0) * 1000 },
    cost: [],
    status: "active",
    enabled: true,
    ...
    Effect.gen(function* () {
      ...
      const diagnostic = (item: Diagnostic) =>
        Effect.gen(function* () {
          ...
          yield* Effect.logWarning("BharatCode catalog record excluded", {
      ...
      const fetchCatalog = Effect.fn("BharatCodeCatalog.fetch")(function* () {
        ...
        for (const item of data) {
          ...
          if (parsed.diagnostic) yield* diagnostic(parsed.diagnostic)
