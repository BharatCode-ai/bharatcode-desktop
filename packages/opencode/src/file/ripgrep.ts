...
/**
 * SHA-256 of each upstream release archive for VERSION.
 *
 * The archive is extracted and then executed, so TLS alone is not enough: a
 * compromised release, a hijacked mirror, or a TLS-intercepting proxy would
 * otherwise hand us an executable we run with the user's privileges, cached
 * for every later session. Verifying against a digest pinned here means the
 * bytes must match what was reviewed when VERSION was chosen.
 *
 * Both values must be updated together when VERSION changes. Upstream
 * publishes `<archive>.sha256` beside each asset.
 */
const PLATFORM = {
  "arm64-darwin": {
    ...
    Effect.gen(function* () {
      ...
      const filepath = yield* Effect.cached(
        Effect.gen(function* () {
          ...
          // Verify before anything touches the disk: the archive is extracted and
          // then executed, so unverified bytes must never be written.
          const digest = createHash("sha256").update(new Uint8Array(bytes)).digest("hex")
          if (digest !== config.sha256) {
            return yield* Effect.fail(
              new Error(`ripgrep archive failed checksum verification: expected ${config.sha256}, got ${digest}`),
            )
          }
