...
export namespace GoalAssessment {
  ...
  export function hasUserAbortedTool(input: { messages: SessionV1.WithParts[]; recentMessages?: number }) {
  ...
  export function assessmentNotes(input: { messages: SessionV1.WithParts[]; assistant?: SessionV1.WithParts }) {
    const notes: string[] = []
    if (input.assistant) {
