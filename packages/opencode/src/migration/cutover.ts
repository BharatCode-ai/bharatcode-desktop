...
async function moveRole(source: string, destination: string, ignoreDestination = maintenanceEntry) {
  ...
  // Sorted so publication order is deterministic. readdir order is filesystem
  // dependent, which would make the set of reachable partial states arbitrary
  // after a crash; sorting makes any partial publication a prefix of a known
  // order, which is what the retry path reasons about.
  const sourceEntries = (await readdir(source)).toSorted()
