...
function fake(
  ...
  return {
    ...
    blockToolCall: () => Effect.die("unused"),
    ...
    () => {
      ...
      return Effect.gen(function* () {
        ...
        yield* ssn.updatePart({
          ...
          sessionID: session.id,
          type: "tool",
          callID: "overflow-progress-tool",
          tool: "bash",
          state: {
            status: "completed",
            input: { command: "synthetic-check" },
            output: "Regression check: upload boundary still fails at 1048576 bytes.",
        ...
        expect(captured).toContain("upload boundary still fails at 1048576 bytes")

        const all = yield* ssn.messages({ sessionID: session.id })
        const replay = all.at(-1)
        expect(replay?.info.role).toBe("user")
        expect(replay?.parts.some((part) => part.type === "tool")).toBe(false)
        expect(
          replay?.parts.some(
            (part) => part.type === "text" && part.text.includes("Implement the streaming upload fix"),
          ),
        ).toBe(true)
