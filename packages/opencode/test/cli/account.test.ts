...
describe("BharatCode account CLI", () => {
  test("temporary account failures keep the session and do not start authorization", async () => {
    const result = await Effect.runPromise(
      ensureSignedIn().pipe(
        Effect.provide(
          Layer.mock(BharatCodeAccount.Service, {
            status: () => Effect.succeed({ state: "connection-problem", message: "seeded-secret" }),
          }),
        ),
        Effect.flip,
      ),
    )
    expect(result.message).toContain("auth reconnect")
    expect(result.message).not.toContain("seeded-secret")
    expect(result.message).not.toContain("auth login")
  })
