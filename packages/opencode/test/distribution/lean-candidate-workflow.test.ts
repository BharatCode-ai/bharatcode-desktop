...
const acceptedReleaseParentSha = "78c401a5808342338c367d53df2685fc564b62d2"
  ...
  try {
    ...
    const execution = Bun.spawnSync(["bun", "--eval", script], {
      ...
      env: {
        ...
        RELEASE_TAG: "desktop-beta-1.15.26",
        WSL_ACCEPTANCE_MODE: "owner-waived-hotfix-1.15.26",
        UPGRADE_ACCEPTANCE_MODE: "owner-waived-hotfix-1.15.26",
      },
      stdout: "pipe",
      stderr: "pipe",
    })
...
describe("lean next-beta candidate workflow", () => {
  test("is manual-only and binds one exact 1.15.26 source plus a fresh WSL decision", async () => {
    ...
    expect(workflow.on.workflow_dispatch.inputs.wsl_acceptance_mode).toEqual({
      description: "Require formal WSL2 automation, or record a fresh owner-authorized 1.15.26 waiver",
      required: true,
      default: "required",
      type: "choice",
      options: ["required", "owner-waived-hotfix-1.15.26"],
    })
    expect(workflow.on.workflow_dispatch.inputs.upgrade_acceptance_mode).toEqual({
      description:
        "Require packaged Windows upgrade/rollback automation, or record the owner-authorized 1.15.26 waiver",
      required: true,
      default: "required",
      type: "choice",
      options: ["required", "owner-waived-hotfix-1.15.26"],
    ...
    expect(value).toContain("github.ref == 'refs/heads/codex/desktop-1.15.26-release-control'")
    expect(value).toContain("github.sha")
    expect(value).toContain("inputs.source_sha")
    expect(value).toContain("ref: ${{ inputs.source_sha }}")
    expect(value).toContain("RELEASE_TAG: desktop-beta-1.15.26")
  ...
  test("uses the exact checked-in current-beta fixture for packaged upgrade acceptance", async () => {
    ...
    expect(fixture.tag).toBe("desktop-beta-1.15.25")
    expect(fixture.source_sha).toBe("b44709aa0e94d66beb3d1d396ba782be2d69a076")
  ...
  test("records the one-release owner waiver without claiming formal WSL scenarios passed", async () => {
    ...
    expect(job.if).toBe("inputs.wsl_acceptance_mode == 'owner-waived-hotfix-1.15.26'")
  ...
  test("records the one-release upgrade waiver without claiming upgrade acceptance passed", async () => {
    ...
    expect(workflow.jobs["record-upgrade-waiver"].if).toBe(
      "inputs.upgrade_acceptance_mode == 'owner-waived-hotfix-1.15.26'",
    )
    ...
    expect(cohortCondition).toContain(
      "inputs.upgrade_acceptance_mode == 'owner-waived-hotfix-1.15.26' && needs.upgrade-rollback-windows-x64.result == 'skipped' && needs.record-upgrade-waiver.result == 'success'",
    )
    const run = runStep(value, "record-upgrade-waiver", "Record exact owner-authorized Windows upgrade waiver")
    expect(run).toContain('result: "OWNER_WAIVED"')
    expect(run).toContain('reason: "WINDOWS_UPGRADE_ROLLBACK_ACCEPTANCE_WAIVED_BY_OWNER_FOR_1_15_26"')
  ...
  test("publishes only an exact complete draft cohort before prerelease and website cutover", async () => {
    ...
    expect(value).toContain("PREVIOUS_RELEASE_TAG: desktop-beta-1.15.25")
    expect(refuseRun).toContain('value.targetCommitish !== "b44709aa0e94d66beb3d1d396ba782be2d69a076"')
