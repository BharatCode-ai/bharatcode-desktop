...
const fsUtilLayer = LayerNode.compile(FSUtil.node)
...
function authLayer(
  root: string,
  filesystem: Layer.Layer<FSUtil.Service> = fsUtilLayer,
...
function runAuth<A, E>(
  ...
  filesystem?: Layer.Layer<FSUtil.Service>,
  options?: AuthLayerOptions,
) {
...
function wrappedFilesystem(
  wrap: (filesystem: FSUtil.Interface) => FSUtil.Interface,
): Layer.Layer<FSUtil.Service> {
  return Layer.effect(
    FSUtil.Service,
    Effect.gen(function* () {
      return FSUtil.Service.of(wrap(yield* FSUtil.Service))
    }),
  ).pipe(Layer.provide(fsUtilLayer))
}
