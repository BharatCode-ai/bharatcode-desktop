...
async function applyPinnedBetaSchema(database: Database) {
  ...
  // A frozen copy of the schema as shipped to beta users. It used to be built by
  // replaying packages/opencode/migration/*.sql, but upstream moved that lineage
  // into programmatic modules under core/database, so those files are gone. It
  // has to stay pinned regardless: the assertions below do positional INSERTs,
  // so tracking the live schema would break them every time a column is added.
  database.run(await readFile(path.join(import.meta.dir, "../fixture/pinned-beta-schema.sql"), "utf8"))
}
