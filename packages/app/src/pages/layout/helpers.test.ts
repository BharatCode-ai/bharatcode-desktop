...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("bharatcode://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  ...
  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "bharatcode://open-project?directory=/a",
      "opencode://other?directory=/b",
      "bharatcode://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("bharatcode://new-session?directory=/tmp/demo")).toEqual({
      directory: "/tmp/demo",
    })
    expect(parseNewSessionDeepLink("bharatcode://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "bharatcode://new-session?directory=/a",
      "opencode://open-project?directory=/b",
      "bharatcode://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __OPENCODE__: {
        deepLinks: ["bharatcode://open-project?directory=/a"],
      },
    } as unknown as Window & { __OPENCODE__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["bharatcode://open-project?directory=/a"])
