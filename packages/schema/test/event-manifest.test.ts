...
describe("public event manifest", () => {
  ...
  test("uses canonical definitions for current public events", () => {
    ...
    const retained = [SessionV1.Event.PartDelta, SessionV1.Event.Diff, SessionV1.Event.Error]
    for (const definition of retained) expect(EventManifest.Latest.get(definition.type)).toBe(definition)
    // Upstream added three durable revert events after this test's original counts.
    for (const definition of Object.values(SessionEvent.RevertEvent)) {
      expect(EventManifest.Latest.get(definition.type)).toBe(definition)
      expect(EventManifest.Durable.get(`${definition.type}.1`)).toBe(definition)
    }
