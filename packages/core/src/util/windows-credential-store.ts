...
public static class BharatCodeCredentialFile {
  ...
  public static void Prepare(string input) {
    using (var parent = PinParent(Full(input), true)) {
      // Only newly created storage receives private ACLs. Existing directories
      // remain unchanged; credential operations still validate their ACLs.
      if (parent.Created) Security(parent.Parent);
    }
  }
