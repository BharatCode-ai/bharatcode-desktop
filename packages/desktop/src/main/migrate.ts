...
const TAURI_APP_IDS: Record<string, string> = {
  dev: appIdForChannel("dev"),
  beta: appIdForChannel("beta"),
  prod: appIdForChannel("prod"),
}
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : appIdForChannel("dev")
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `bharatcode.settings.dat` is special: it maps to the `bharatcode.settings` store
...
function migrateFile(datPath: string, filename: string) {
  ...
  // bharatcode.settings.dat → the electron settings store ("bharatcode.settings").
