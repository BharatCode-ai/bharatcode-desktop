...
export function createAccountSession(options: {
  ...
  const cancel = () => {
    ...
    pending = undefined
    attempt?.clear()
    attempt?.reject(new Error("BharatCode sign-in was cancelled. Try again."))
  }
  ...
  const getAccountStatus = async () => {
    ...
    const next = await options.client.getAccountStatus().catch(() => {
      ...
      throw new Error("Could not check the BharatCode account. Try again.")
    })
    if (epoch !== generation || read !== reads || disposed) return latest
    return publish(next)
  }
  ...
  const beginSignIn = (input: { selectAccount?: boolean } = {}) => {
    if (disposed) return Promise.reject(new Error("The BharatCode account runtime is unavailable."))
    cancel()
    ...
    attempt.clear = schedule(() => fail(attempt, "Timed out waiting for BharatCode sign-in. Try again."), 180_000)
    void serial(async () => {
      if (pending !== attempt) return
    ...
    }).catch(() => fail(attempt, "Could not start BharatCode sign-in. Try again."))
  ...
  const completeSignIn = (callbackUrl: string) => {
    ...
    return serial(async () => {
      ...
      fail(attempt, "BharatCode sign-in could not be completed. Try again.")
      throw new Error("BharatCode sign-in could not be completed. Try again.")
    })
  }
  ...
  const logout = () => {
    ...
    return serial(async () => {
      ...
      .catch(() => {
        if (epoch === generation && !disposed) state("connection_issue", "Could not sign out of BharatCode. Try again.")
  ...
  return {
    ...
    cancelSignIn: () => {
      if (!pending) return
      cancel()
      state("needs_sign_in", "BharatCode sign-in was cancelled. Try again.")
    },
  }
}
