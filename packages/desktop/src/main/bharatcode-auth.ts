...
export function createBharatCodeAccountClient(options: ClientOptions) {
  ...
  const request = async (path: string, init: RequestInit = {}) => {
    ...
    if (!response.ok) throw new SidecarRequestError(response.status, nativeT("desktop.account.error.request"))
    return value
  }
  ...
  const getAccountStatus = async () => {
    try {
      ...
      return {
        ...
        message: nativeT("desktop.account.error.storage"),
...
function projectStatus(input: unknown, checkedAt: Date): BharatCodeAccountStatus {
  ...
  if (input.state === "sign-in-required")
    return {
      ...common,
      state: "needs_sign_in",
      authenticated: false,
      message: nativeT("desktop.account.error.signInRequired"),
    }
  if (input.state === "connection-problem")
    return {
      ...common,
      state: "connection_issue",
      authenticated: true,
      message: nativeT("desktop.account.error.connection"),
    }
