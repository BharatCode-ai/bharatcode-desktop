...
describe("real packaged Windows upgrade and rollback acceptance", () => {
  ...
  test("ships explicit real installer, process, timeout, cleanup, state, recovery, and network boundaries", async () => {
    ...
    expect(source).toContain('diagnosticStage = "CANDIDATE_RECOVERY_STATUS"')
    expect(source).toContain('diagnosticStage = "CANDIDATE_RECOVERY_CHOOSE"')
    expect(source).toContain('diagnosticStage = "CANDIDATE_RECOVERY_EVIDENCE"')
