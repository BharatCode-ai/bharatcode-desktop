...
const releaseId = process.env.BHARATCODE_RELEASE ?? process.env.OPENCODE_RELEASE
if (!releaseId) throw new Error("BHARATCODE_RELEASE is required")

const version = process.env.BHARATCODE_VERSION ?? process.env.OPENCODE_VERSION
if (!version) throw new Error("BHARATCODE_VERSION is required")
...
const macxTarGz = "bharatcode-desktop-mac-x64.app.tar.gz"
const macaTarGz = "bharatcode-desktop-mac-arm64.app.tar.gz"
